...
<div class="space-y-6 max-w-6xl mx-auto">
    ...
    <div class="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md flex flex-col md:flex-row items-center md:items-start gap-6 relative overflow-hidden">
        ...
        <div class="flex items-center gap-2 self-center shrink-0">
            <a href="{{ route('employees.edit', $employee->id) }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-semibold shadow transition cursor-pointer">
                Edit Profile
            </a>
            <a href="{{ route('employees.index') }}" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-xl text-xs font-semibold transition cursor-pointer">
                Directory
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <div id="tab-profile" class="tab-content bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-md space-y-6">
                ...
                <div class="space-y-4 pt-4 border-t border-white/5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                            ...
                            <span class="block font-semibold text-white mt-1">{{ $employee->employeeDetail?->gender ?? 'N/A' }}</span>
                        </div>
                        <div>
                ...
                <!-- Group 3: Financial & Statutory Info -->
                <div class="space-y-4 pt-4 border-t border-white/5">
                    <h4 class="text-sm font-bold text-indigo-400 uppercase tracking-widest border-b border-white/5 pb-2">Financial Registry</h4>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div>
                            <span class="block text-slate-500 font-medium">Bank Name</span>
                            <span class="block font-semibold text-white mt-1">{{ $employee->employeeDetail?->bank_name ?? 'N/A' }}</span>
                        </div>
                        <div>
                            <span class="block text-slate-500 font-medium">Bank Account Number</span>
                            <span class="block font-semibold text-white mt-1">
                                {{ $employee->employeeDetail?->bank_account_no ?? 'N/A' }}
                            </span>
                        </div>
                        <div>
                            <span class="block text-slate-500 font-medium">PAN/Tax Registration</span>
                            <span class="block font-semibold text-white mt-1">{{ $employee->employeeDetail?->pan_no ?? 'N/A' }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="divide-y divide-white/5">
                    @forelse($employee->documents as $doc)
                        <div class="py-3 flex items-center justify-between text-sm">
                            ...
                            <a href="#" class="px-3 py-1.5 rounded-lg bg-slate-850 hover:bg-slate-800 text-slate-300 font-medium text-xs border border-slate-700 transition">
                                Download
                            </a>
                        </div>
                    @empty
                        <div class="py-8 text-center text-slate-500 text-sm">
                            No files uploaded to this employee's profile yet.
                        </div>
                    @endforelse
                </div>
            </div>
