...
@section('page_title', 'Edit Department: ' . $department->department_name)
        ...
        <form action="{{ route('departments.update', $department->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="department_code" id="department_code" required value="{{ old('department_code', $department->department_code) }}"
                ...
                <div>
                    ...
                    <input type="text" name="department_name" id="department_name" required value="{{ old('department_name', $department->department_name) }}"
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm" 
                ...
                <div>
                    ...
                    <select name="head_employee_id" id="head_employee_id"
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ old('head_employee_id', $department->head ? $department->head->user_id : '') == $emp->id ? 'selected' : '' }}>{{ $emp->name }} ({{ $emp->email }})</option>
                        @endforeach
                    </select>
                </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        textarea.value = `{!! addslashes($department->description) !!}`;
    });
</script>
@endsection
