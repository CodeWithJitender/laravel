        ...
        <form action="{{ route('shifts.update', $shift->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="shift_code" id="shift_code" required value="{{ old('shift_code', $shift->shift_code) }}"
                ...
                <div>
                    ...
                    <input type="text" name="shift_name" id="shift_name" required value="{{ old('shift_name', $shift->shift_name) }}"
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="time" name="start_time" id="start_time" required value="{{ old('start_time', substr($shift->start_time, 0, 5)) }}"
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm">
                </div>

                <div>
                    <label for="end_time" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">End Time</label>
                    <input type="time" name="end_time" id="end_time" required value="{{ old('end_time', substr($shift->end_time, 0, 5)) }}"
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="number" name="grace_period_minutes" id="grace_period_minutes" required value="{{ old('grace_period_minutes', $shift->grace_period_minutes) }}"
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm" 
                        placeholder="15">
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="break_minutes" id="break_minutes" required value="{{ old('break_minutes', $shift->break_minutes) }}"
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm" 
                        placeholder="60">
                </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        textarea.value = `{!! addslashes($shift->description) !!}`;
    });
</script>
@endsection
