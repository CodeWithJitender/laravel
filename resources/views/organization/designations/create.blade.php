        ...
        <form action="{{ route('designations.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="designation_code" id="designation_code" required value="{{ old('designation_code') }}"
                ...
                <div>
                    ...
                    <input type="text" name="designation_name" id="designation_name" required value="{{ old('designation_name') }}"
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm" 
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <select name="level" id="level" required
                        ...
                        <option value="1" {{ old('level') == '1' ? 'selected' : '' }}>Level 1 (CEO / Top)</option>
                        <option value="2" {{ old('level') == '2' ? 'selected' : '' }}>Level 2 (Director)</option>
                        <option value="3" {{ old('level') == '3' ? 'selected' : '' }}>Level 3 (Manager)</option>
                        <option value="4" {{ old('level') == '4' ? 'selected' : '' }}>Level 4 (Team Lead)</option>
                        <option value="5" {{ old('level', '5') == '5' ? 'selected' : '' }}>Level 5 (Employee)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="parent_designation_id" id="parent_designation_id"
                        ...
                        @foreach($parentDesignations as $pDesg)
                            <option value="{{ $pDesg->id }}" {{ old('parent_designation_id') == $pDesg->id ? 'selected' : '' }}>{{ $pDesg->designation_name }} (Level {{ $pDesg->level }})</option>
                        @endforeach
                    </select>
                </div>
