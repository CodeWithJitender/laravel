                ...
                @forelse($locations as $loc)
                    <tr class="hover:bg-white/2 transition duration-150">
                        <td class="px-6 py-4 w-10">
                            <input type="checkbox" class="row-checkbox w-4 h-4 rounded border-slate-700 text-indigo-600 focus:ring-indigo-500 bg-slate-900 cursor-pointer" value="{{ $loc->id }}">
                        </td>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        window.confirmBulkDelete = async () => {
            ...
            if (confirm(`Are you sure you want to delete the ${selectedIds.length} selected location(s)?`)) {
                try {
                    const response = await fetch('{{ route("locations.bulk_destroy") }}', {
                        method: 'POST',
                        headers: {
        ...
        bulkEditForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('{{ route("locations.bulk_update") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}',
