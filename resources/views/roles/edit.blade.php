...
<div class="max-w-4xl mx-auto">
    ...
    <div class="backdrop-blur-md bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden mb-6">
        ...
        <h2 class="text-xl font-semibold mb-6 text-slate-200">Modify Role Settings</h2>
        ...
        <form action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" required value="{{ old('name', $role->name) }}"
                        {{ in_array($role->name, ['Admin', 'Manager', 'Employee']) ? 'readonly' : '' }}
                        class="w-full bg-slate-900/60 border border-white/10 rounded-2xl py-3 px-4 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200 text-sm {{ in_array($role->name, ['Admin', 'Manager', 'Employee']) ? 'opacity-60 cursor-not-allowed' : '' }}" 
            ...
            <div>
                <h3 class="text-sm font-semibold uppercase tracking-wider text-slate-300 mb-4 border-b border-white/5 pb-2">Modify Assigned Permissions</h3>
